...
export const farmerApi = {
  ...
  /** Get all mandis with coordinates */
  getMandis: async (): Promise<Array<{
    id: string;
    name: string;
    district: string;
    ...
    const res = await fetch(`${API_URL}/mandis`);
    return handleResponse(res);
  },

  /** Get single mandi by ID with coordinates */
  getMandiById: async (mandiId: string): Promise<{
    id: string;
    name: string;
    district: string;
    daily_capacity?: number;
    ...
    const res = await fetch(`${API_URL}/mandis/${mandiId}`);
    return handleResponse(res);
  },
};
