...
export default function AdminConsolePage() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      {feedback && (
        <div className="p-4 bg-amber-50 border border-amber-300 rounded-xl text-xs font-bold text-amber-950 flex justify-between items-center shadow-xs">
          <span>{feedback}</span>
          <button onClick={() => setFeedback(null)} className="text-amber-800 font-extrabold cursor-pointer">✕</button>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-4">
        <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
          <span>⚙️</span> Procurement Centre Configuration Parameters
        </h2>
        <form onSubmit={handleSaveConfig} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-3">
            <button
              type="submit"
              className="bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold py-2.5 px-6 rounded-lg shadow-xs transition-colors cursor-pointer"
            >
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-4">
        <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
          <span>👥</span> Staff User &amp; Role Permission Management
        </h2>
