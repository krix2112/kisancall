...
import { INITIAL_CALLS, CallRecord } from '@/lib/staffMockData';
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {toast && (
        <div className="p-3 bg-sky-50 border border-sky-200 rounded-lg text-xs font-semibold text-sky-900 flex justify-between items-center">
          <span>{toast}</span>
          <button onClick={() => setToast(null)} className="text-sky-700 hover:text-sky-950 font-bold cursor-pointer">✕</button>
        </div>
      )}
            ...
            <tr key={item.id} className="hover:bg-slate-50">
              ...
              <td className="py-3 px-4 text-right space-x-2">
                <button
                  ...
                  className="bg-slate-100 hover:bg-slate-200 text-slate-700 border text-xs font-semibold py-1.5 px-2.5 rounded-lg transition-colors cursor-pointer"
                >
                  🔄 Retry Call
                </button>
                <button
                  onClick={() => handleEscalateVisit(item.id, item.farmerName)}
                  className="bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold py-1.5 px-2.5 rounded-lg shadow-sm transition-colors cursor-pointer"
                >
                  🚩 Escalate
                </button>
              </td>
