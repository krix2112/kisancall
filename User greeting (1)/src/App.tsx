...
import RegisterProcurement from "./RegisterProcurement";
import Sidebar from "./Sidebar";
import farmerImg from "@/imports/image-1.png";
import callFarmerImg from "@/imports/image-4.png";
import {
  FileText, CreditCard, Phone,
  MapPin, Bell, ChevronDown, ChevronRight, Mic,
  PhoneCall, Search, Leaf, Wheat, Banknote, History, Settings,
  TrendingUp, CheckCircle2, Circle, Navigation, Bot, Sprout, Zap,
  Shield
} from "lucide-react";

const HERO_BG = "https://images.unsplash.com/photo-1642863742974-7e0e40c8c0c9?w=1400&h=400&fit=crop&auto=format";
...
const services = [
  { icon: TrendingUp, label: "Mandi Price", sub: "MSP & Rates", color: "bg-green-50", iconColor: "text-green-600", glow: "#22c55e" },
  { icon: CreditCard, label: "Payment Status", sub: "PFMS Status", color: "bg-blue-50", iconColor: "text-blue-600", glow: "#3b82f6" },
  { icon: FileText, label: "Procurement", sub: "J-Form Slip", color: "bg-purple-50", iconColor: "text-purple-600", glow: "#a855f7" },
  { icon: History, label: "Call History", sub: "Voice Logs", color: "bg-pink-50", iconColor: "text-pink-600", glow: "#ec4899" },
  { icon: Settings, label: "KYC & Settings", sub: "Manage Profile", color: "bg-gray-50", iconColor: "text-gray-600", glow: "#6b7280" },
];
...
function ProgressRing({ value, max }: { value: number; max: number }) {
  ...
  return (
    <svg width="120" height="120" viewBox="0 0 120 120" className="relative">
      ...
      />
      <text x="60" y="52" textAnchor="middle" fontSize="28" fontWeight="800" fill="#2D6A4F" fontFamily="Poppins">12</text>
...
type Screen = "dashboard" | "mandi" | "payment" | "bookslot";
...
export default function App() {
  ...
  if (screen === "mandi") {
    return <MandiPrices onNavigate={(s) => setScreen(s as Screen)} />;
  }
  if (screen === "payment") {
    return <Payment onNavigate={(s) => setScreen(s as Screen)} />;
  }
  if (screen === "bookslot") {
    return <RegisterProcurement onNavigate={(s) => setScreen(s as Screen)} />;
  }

  return (
    <div className="flex h-screen overflow-hidden bg-[#FDFBF5]" style={{ fontFamily: "'Poppins', sans-serif" }}>
      <Sidebar activeScreen="dashboard" onNavigate={(s) => setScreen(s as Screen)} />
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="relative rounded-3xl overflow-hidden h-[200px] shadow-xl">
            {/* Full background: farmer photo, blurred */}
            <img
              src={farmerImg}
            ...
            />
            {/* Light overlay — keeps text readable without heavy darkness */}
            ...
            <div className="relative z-10 flex items-center justify-between h-full px-8">
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-2">
                  ...
                  <span className="text-[12px] text-[#5a7a68] font-medium">Rampur, Uttar Pradesh</span>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            <div className="card-hover bg-white rounded-3xl p-5 shadow-[0_4px_24px_0_#2D6A4F12] border border-[#E8F0E8]">
              ...
              <div className="flex items-center gap-6">
                <ProgressRing value={12} max={20} />
                <div>
                  <p className="text-[11px] text-[#6B6B6B] mb-0.5">Estimated Time</p>
                  <p className="text-[22px] font-extrabold text-[#2B2B2B] leading-tight">~45 minutes</p>
                  <div className="flex items-center gap-1.5 mt-2">
                    <Wheat size={13} className="text-[#D9A441]" />
                    <p className="text-[12px] font-semibold text-[#2B2B2B]">Rampur Mandi</p>
                  </div>
                  <p className="text-[11px] text-[#6B6B6B]">Wheat | 2 Apr 2026</p>
                </div>
              </div>
            ...
            <div className="card-hover bg-white rounded-3xl overflow-hidden shadow-[0_4px_24px_0_#2D6A4F12] border border-[#E8F0E8]">
              ...
              <div className="px-5 pb-2">
                <p className="text-[13px] font-bold text-[#2B2B2B]">Rampur Mandi</p>
                <p className="text-[11px] text-[#6B6B6B]">District: Rampur, Uttar Pradesh</p>
              </div>
          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            <div className="card-hover bg-white rounded-3xl shadow-[0_4px_24px_0_#2D6A4F12] border border-[#E8F0E8] relative overflow-hidden">
              {/* Wheat photo — decorative accent, clipped by overflow-hidden on parent */}
              <div className="absolute right-0 top-0 h-full w-[28%] pointer-events-none">
                <img
                  ...
                  style={{ opacity: 0.75 }}
                />
                {/* Wide soft fade so image never bleeds into text */}
              ...
              {/* Text content — right padding keeps it clear of the image zone */}
            ...
            {/* Payment Status */}
            <div className="card-hover bg-white rounded-3xl shadow-[0_4px_24px_0_#2D6A4F12] border border-[#E8F0E8] relative overflow-hidden">
              {/* Rupee photo — low opacity, clipped by overflow-hidden */}
              <div className="absolute right-0 top-0 h-full w-[28%] pointer-events-none">
                <img
                  src={RUPEE_IMG}
                  alt="Rupee coins"
          ...
          {/* ── Call Us Banner (image-2 style) ── */}
          <div className="relative rounded-3xl overflow-hidden shadow-lg bg-[#E8F5EE] border border-[#C3E8CC]" style={{ minHeight: "120px" }}>
            {/* Leaf decorations */}
            <div className="absolute right-[280px] top-0 opacity-25 pointer-events-none select-none">
              <Leaf size={90} className="text-[#2D6A4F]" strokeWidth={1} />
            </div>
            <div className="absolute right-[240px] bottom-0 opacity-15 pointer-events-none select-none rotate-180">
              <Leaf size={70} className="text-[#2D6A4F]" strokeWidth={1} />
            </div>

            {/* Farmer fills the whole banner */}
            <img
              src={callFarmerImg}
            ...
            />
            {/* Overlay: solid on left, fades right */}
            ...
            <div className="relative z-10 flex items-center h-full px-8 py-6 gap-5">
              ...
              {/* Animated sound wave */}
              <div className="flex items-end gap-[3px] h-10 ml-6 flex-shrink-0">
                {[0.4, 0.7, 1, 0.85, 1, 0.65, 0.45, 0.8, 1, 0.6, 0.4].map((h, i) => (
                  <div
                    key={i}
