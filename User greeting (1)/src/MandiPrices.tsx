...
export default function MandiPrices({ onNavigate }: { onNavigate?: (s: string) => void }) {
  const [showTooltip, setShowTooltip] = useState(false);

  return (
    <div className="flex h-screen overflow-hidden bg-[#FDFBF5]" style={{ fontFamily: "'Poppins', sans-serif" }}>

      <Sidebar activeScreen="mandi" onNavigate={(s) => onNavigate?.(s)} />

      {/* ── Main ── */}
      <div className="flex-1 flex flex-col overflow-hidden">

        {/* ── Top bar ── */}
        <header className="flex items-center gap-3 px-5 py-3 bg-[#FDFBF5] border-b border-[#E8F0E8] flex-shrink-0 z-10">
          <div className="flex-1 flex items-center gap-2.5 bg-[#F4F8F4] border border-[#E0EDE3] rounded-full px-4 py-2">
            <Search size={14} className="text-[#9AB4A0]" />
            <input className="flex-1 bg-transparent text-[12px] text-[#6B6B6B] placeholder-[#9AB4A0] outline-none"
              placeholder="Search for crops, mandis, districts or ask anything..." />
          </div>
          <button className="flex items-center gap-1.5 bg-[#EAF5EE] border border-[#C3E8CC] text-[#2D6A4F] text-[12px] font-bold rounded-full px-4 py-2">
            <Mic size={13} /> Ask KisanCall AI
            <Leaf size={11} />
          </button>
          <button className="flex items-center gap-1 text-[12px] font-semibold text-[#2B2B2B] bg-[#F4F8F4] border border-[#E0EDE3] rounded-full px-3 py-2">
            <Globe size={13} className="text-[#6B6B6B]" />
            EN | हिंदी <ChevronDown size={12} />
          </button>
          <button className="relative w-9 h-9 rounded-full bg-[#F4F8F4] border border-[#E0EDE3] flex items-center justify-center"
            ...
            <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 bg-red-500 rounded-full border-2 border-white text-[7px] text-white flex items-center justify-center font-bold">3</span>
            {showTooltip && (
              <div className="absolute top-11 right-0 bg-white rounded-2xl shadow-2xl border border-[#E8F0E8] p-3 w-52 text-left z-20">
                ...
                {["Wheat rate updated in Rampur","MSP revision announced","New mandi added nearby"].map((n,i) => (
          ...
          <div className="flex items-center gap-2 cursor-pointer group">
            <div className="w-9 h-9 rounded-full bg-[#2D6A4F] flex items-center justify-center text-white text-[12px] font-bold" style={{ boxShadow: "0 2px 8px #2D6A4F44" }}>
              RK
            </div>
            <div>
